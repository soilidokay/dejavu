...
if __name__ == '__main__':
    ...
    # djv.fingerprint_directory("test2", [".mp3"])
    ...
    if len(results["results"]) > 0:
        ...
        song_path = f'test2/{song_name}.mp3'
        songs = [song_path, recognize_file]

        for song_q in results["results"][0]["offsets"]:
            start_time = 0
            for idex, song_seg in enumerate(song_q):
                for idx2, song in enumerate(song_seg):
                    ...
                    file_out = f"{idex}_{idx2}_{start_time}_{end_time}_output_segment.mp3"
